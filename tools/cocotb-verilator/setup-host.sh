...
#   $CORAL_WORK/venv       cocotb 2.0.0, pyelftools, numpy, tqdm
...
[ -d "$HOME/miniforge3/envs/vl" ] || conda create -y -q -n vl -c conda-forge "verilator=5.050" ccache sbt openjdk=17
[ -x "$CORAL_PY" ] || /usr/bin/python3 -m venv "$CORAL_WORK/venv"
"$CORAL_PY" -m pip install -q "cocotb==2.0.0" pyelftools numpy tqdm   # 2.0.0: Coral pins it; 2.1 changes handle types (PackedObject)
...
verilator --version; "$CORAL_PY" -c "import cocotb; print('cocotb', cocotb.__version__)"; "$CORAL_TC/riscv32-unknown-elf-gcc" --version | head -1
